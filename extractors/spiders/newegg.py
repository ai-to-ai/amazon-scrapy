...
class NewEggSpider(scrapy.Spider):
	...
	def parse_product(self, response):
		...
		# imageLink = response.xpath('//div[@class="product-view-container"]/div[@style="display: none;"]/img/@src').getall()
		imageLink = response.xpath('//img[@style="margin:auto;transform:scale(1);transform-origin:top left;transition-duration:300ms;opacity:1"]/@src').getall()
